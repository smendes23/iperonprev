...
public class AtoAposentadoriaInvalidez implements AtoConcessorio{
	...
	public AtoConcessorioDto redirecionaAto(AtosConcessorios atosConcessorios,PoderPessoas chefePoder,PoderPessoas presidenteIperon) {
		try{
			...
			if(pf.getENUM_tipoAposentadoria() == TipoBeneficioEnum.INVALIDEZ){
			AposentadoriaPorInvalidez iv = new GenericPersistence<AposentadoriaPorInvalidez>(AposentadoriaPorInvalidez.class).buscaObjetoRelacionamento("AposentadoriaPorInvalidez", "NUMR_idDoObejtoPessoasFuncionais", pf.getNUMG_idDoObjeto());
			...
			for (int i = 0; i < iv.getREL_atoLegais().size(); i++) {
				atoLegal.append(iv.getREL_atoLegais().get(i).getDESC_tituloEmenta());
				if(i != iv.getREL_atoLegais().size()-1){
					atoLegal.append(";");
				}else{
					atoLegal.append(".");
				}
			}
				dto.setNumeroProcesso(iv.getNUMR_processo());
				...
				if(iv.getENUM_proventos() == TipoProventosEnum.INTEGRAL){
					proventos = "integrais";
				}else{
				...
				dto.setTexto1(
						new StringBuilder().append("1 - Conceder aposentadoria  por Invalidez com proventos ").append(proventos).append(" ao(a) servidor(a) ")
