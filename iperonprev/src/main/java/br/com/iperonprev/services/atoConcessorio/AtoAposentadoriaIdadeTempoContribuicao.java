...
public class AtoAposentadoriaIdadeTempoContribuicao implements AtoConcessorio{
	...
	public AtoConcessorioDto redirecionaAto(AtosConcessorios atosConcessorios,PoderPessoas chefePoder,PoderPessoas presidenteIperon) {
		try{
			...
			if(pf.getENUM_tipoAposentadoria() == TipoBeneficioEnum.IDADETEMPO){
			...
			for (int i = 0; i < it.getREL_atoLegais().size(); i++) {
				atoLegal.append(it.getREL_atoLegais().get(i).getDESC_tituloEmenta());
				if(i != it.getREL_atoLegais().size()-1){
					atoLegal.append(";");
				}else{
					atoLegal.append(".");
				}
				...
				dto.setNumeroProcesso(it.getNUMR_processo());
				...
				if(it.getENUM_proventos() == TipoProventosEnum.INTEGRAL){
					proventos = "integrais";
				}else{
				...
				dto.setTexto1(
						new StringBuilder().append("1 - Conceder aposentadoria  voluntária por idade e tempo de contribuição com proventos ").append(proventos).append(" ao(a) servidor(a) ")
