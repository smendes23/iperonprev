...
public class AtoAposentadoriaCompulsoria implements AtoConcessorio{
	...
	public AtoConcessorioDto redirecionaAto(AtosConcessorios atosConcessorios,PoderPessoas chefePoder,PoderPessoas presidenteIperon) {
		try{
			...
			if(pf.getENUM_tipoAposentadoria() == TipoBeneficioEnum.COMPULSORIA){
			AposentadoriaCompulsoria c = new GenericPersistence<AposentadoriaCompulsoria>(AposentadoriaCompulsoria.class).buscaObjetoRelacionamento("AposentadoriaCompulsoria", "NUMR_idDoObejtoPessoasFuncionais", pf.getNUMG_idDoObjeto());
			...
			for (int i = 0; i < c.getREL_atoLegais().size(); i++) {
				atoLegal.append(c.getREL_atoLegais().get(i).getDESC_tituloEmenta());
				if(i != c.getREL_atoLegais().size()-1){
					atoLegal.append(";");
				}else{
					atoLegal.append(".");
				}
				...
				dto.setNumeroProcesso(c.getNUMR_processo());
				...
				if(c.getENUM_proventos() == TipoProventosEnum.INTEGRAL){
					proventos = "integrais";
				}else{
				...
				dto.setTexto1(
						new StringBuilder().append("1 - Conceder aposentadoria  compulsória com proventos ").append(proventos).append(" ao(a) servidor(a) ")
